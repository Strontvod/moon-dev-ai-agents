...
from src.models.model_factory import ModelFactory, model_factory as _global_model_factory
...
- Max single order size: ${max_order}
...
class RebalanceAgent:
    ...
    def __init__(self):
        ...
        if EXCHANGE != 'hyperliquid':
            raise ValueError("Rebalance Agent only supports HyperLiquid exchange")
        ...
        # Use the global ModelFactory singleton (already initialized at import time)
        self.model_factory = _global_model_factory

        # Parse AI_MODEL to get model type and name
        self.ai_model_type, self.ai_model_name = self._parse_ai_model(AI_MODEL)
        cprint(f"  AI model: {self.ai_model_type}/{self.ai_model_name}", "green")
    ...
    def generate_rebalance_orders(self, deviations, signal_fusion):
        ...
        - If fusion agrees with rebalance direction, full rebalance (1.0x)
        - If fusion disagrees, partial rebalance (0.5x)
        - NEUTRAL: full mechanical rebalance

        Returns:
            list of order dicts: {symbol, action, usd_amount, reason, deviation_pct}
        """
        orders = []
        ...
        for symbol, dev in deviations.items():
            ...
            # delta_usd > 0 means overweight → sell
            # delta_usd < 0 means underweight → buy
            delta = dev["delta_usd"]

            # Apply signal fusion bias
            bias_factor = 1.0
            if delta < 0:
                # We want to BUY more
    ...
    def get_llm_confirmation(self, allocations, orders, signal_fusion):
        ...
        Returns:
            tuple: (approved: bool, response_text: str)
        """
        ...
        if not model:
            cprint(f"  Configured model {self.ai_model_type} unavailable, trying fallbacks...", "yellow")
            for fallback_type in ["claude", "openai", "deepseek", "groq", "moondev"]:
                if fallback_type == self.ai_model_type:
                    continue
                model = self.model_factory.get_model(fallback_type)
                if model:
                    cprint(f"  Using fallback model: {fallback_type}", "yellow")
                    break
        ...
        try:
            ...
            if "APPROVE" in first_line:
                ...
                # Partial approval — proceed but log the partial note
                return True, response_text
            else:
    ...
    def log_rebalance(self, allocations, orders, llm_approved, llm_response):
    ...
    def run(self):
        ...
        if not orders:
            ...
            self.log_rebalance(allocations, [], True, "Within threshold")
            return
        ...
        llm_approved, llm_response = self.get_llm_confirmation(allocations, orders, signal_fusion)

        if not llm_approved:
            cprint("\n  LLM REJECTED rebalance. No trades executed.", "yellow")
            self.log_rebalance(allocations, orders, False, llm_response)
            return
        ...
        # 10. Log results
        self.log_rebalance(allocations, executed, True, llm_response)
