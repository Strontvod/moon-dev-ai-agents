...
class SelectiveMomentumReversal(bt.Strategy):
    ...
    def next(self):
        if not self.position:
            # Buy signal
            if self.rsi[-2] >= self.params.rsi_oversold and self.data.Close[-1] <= self.pivot_low[-1]:
                volume_condition = self.data.Volume[-1] > 1.5 * self.avg_volume[-1]
                if volume_condition:
                    print(f"🌙 Entering long position at {self.data.Close[0]}")
                    self.buy(size=self.get_position_size())

            # Sell short signal
            elif self.rsi[-2] <= self.params.rsi_overbought and self.data.Close[-1] >= self.pivot_high[-1]:
                volume_condition = self.data.Volume[-1] > 1.5 * self.avg_volume[-1]
                if volume_condition:
