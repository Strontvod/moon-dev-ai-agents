...
class AdaptiveDivergenceVolatility(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.cross_above = bt.ind.CrossOver(self.macd_hist, 0)
        self.cross_below = bt.ind.CrossUnder(self.macd_hist, 0)

        self.buy_signal = (self.cross_above > 0) & (self.data.Volume > self.data.Volume.rolling(20).mean())
        self.sell_signal = (self.cross_below < 0) & (self.data.Volume > self.data.Volume.rolling(20).mean())
    ...
    def next(self):
        if not self.position:
            if self.buy_signal:
                ...
                print(f"🌙 Entering long position on {self.data.datetime.date(0)} 🚀")
                ...
                print(f"🌙 Entering short position on {self.data.datetime.date(0)} 🔥")
        else:
            if self.macd_hist.crossbelow(0) or self.atr[0] > self.atr.mean():
                self.close()
