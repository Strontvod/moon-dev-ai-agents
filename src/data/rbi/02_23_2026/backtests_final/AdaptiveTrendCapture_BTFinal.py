...
class AdaptiveTrendCapture(bt.Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            if (self.fast_ema[-2] < self.medium_ema[-2] and self.fast_ema[-1] > self.medium_ema[-1]) and self.adx[-1] > self.params.adx_threshold:
                self.trend_direction = 1
                self.log(f'🌙 Bullish trend detected! 🚀')
            elif (self.fast_ema[-2] > self.medium_ema[-2] and self.fast_ema[-1] < self.medium_ema[-1]) and self.adx[-1] > self.params.adx_threshold:
                self.trend_direction = -1
            ...
            if self.trend_direction != 0 and self.data_close[-1] < self.medium_ema[-1]:
                self.buy_size = int(self.broker.cash * self.params.risk_pct / self.atr[-1])
